...
# MAIN ALGO LOOP
def algo_loop(trading_day, order_side, original_order_quantity, vwap_coefficients, schedule_coef, tick_coef,LSTM_coef=0,LSTM=False):
    ...
    for index, row in trading_day.iterrows():
        ...
        if LSTM:
            if row.signal_continuous != np.nan:
                LSTM_factor = row.signal_continuous
            else:
                LSTM_factor = 0
        else:
            LSTM_coef = 0
        ...
        fair_value = midpoint + (schedule_coef * schedule_factor * half_spread) + (
                tick_coef * tick_factor * half_spread) + (LSTM_coef * LSTM_factor * half_spread)
